function [EVs_prox, EC, EVs_db] = EvalProxObj2(S,S_,Path,Px,Pm,Pl,Cd,db)
%{
Compute the measure required for proposed strategy of optimization (using link-sets)

INPUT:
    S: survival nodes of cuts identified by RDA = Nset x 1 cell
    S_: failed nodes of cuts identified by RDA - Nset x 1 cell
    Path: link sets
...
for ff = 1:Nfac
   ...
   c_ = [S{fl}(:)' Path{fl}(:)' S_{fl}(:)']; % failed comp's / survived comp's
   Ns_ = length(S{fl})+length( Path{fl} ); Nc_ = length(c_);
